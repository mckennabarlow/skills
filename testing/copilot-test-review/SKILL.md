...
- **Run metadata (auto-detected)** — if the folder contains a `run-metadata.md` file (created by the `collect-test-copilot-logs` skill), read it to pre-populate the Prompt, Target, Date/Time, Visual Studio Version, and Copilot Chat Version fields. Still confirm with the user.
...
- **Model used** — auto-extract from the Copilot diagnostic log (search for `PreferredModelFamily=` in the config section, typically within the first 350 lines). Only ask the user if the log doesn't contain the model.
