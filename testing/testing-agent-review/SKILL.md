...
- **Run metadata (auto-detected)** — if the folder contains a `run-metadata.md` file (created by the `collect-test-testingagent-logs` skill), read it to pre-populate the Prompt, Target, Date/Time, and Visual Studio Version fields. Still confirm with the user.
