...
description: >
  ...
  Supports creating multiple work items across different area paths in a single invocation.
  Extracts issue titles for the work item title, posts the full markdown as a Discussion comment,
  and maps the first issue's ROI level to ADO Priority. After creation, updates the source
  markdown file with a bullet list of all logged issues showing title, area path, and URL.
...
Look for issue headings in the markdown. These follow the pattern:

```
### Issue N (ROI: HIGH|MEDIUM|LOW): <issue title text>
```

**Single issue:** Extract the text after `ROI: HIGH|MEDIUM|LOW): ` as the title.
- Example: `### Issue 1 (ROI: HIGH): VS ReportService crash terminates run before test generation`
- Title: `VS ReportService crash terminates run before test generation`

**Multiple issues:** Read all issue heading titles and synthesize a short summary (under 120 characters) that captures the overall theme. Do not just concatenate them — write a brief descriptive title.

### Step 4: Extract the Priority

From the **first** issue heading, extract the ROI level and map it to ADO Priority:
...
- **fields:**
  - `System.Title` — the extracted title from Step 3
